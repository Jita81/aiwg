 ...
 * Sync command handler
 */
export const syncHandler: CommandHandler = {
  id: 'sync',
  name: 'Sync',
  description: 'Sync AIWG to latest version and re-deploy all frameworks',
  ...
  async execute(ctx: HandlerContext): Promise<HandlerResult> {
    ...
    if (!quiet) {
      ui.blank();
      console.log(`  ${ui.brandMark()} ${ui.bold('aiwg sync')}${dryRun ? ui.dim('  (dry run)') : ''}`);
      ui.rule();
    }
