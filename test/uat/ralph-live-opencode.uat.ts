/**
 * Ralph External Loop — Live UAT with OpenCode Provider
 *
 * Runs the same orchestrator test cases as the stub UAT suite but using
 * the REAL opencode provider. Requires a valid opencode CLI installation
 * and configured credentials.
 *
 * Run on demand (NOT part of CI):
 *   npm run uat:opencode
 *
 * Requirements:
...
function makeTmpDir(): string {
  const dir = join(tmpdir(), `aiwg-uat-opencode-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`);
...
describe('UAT [LIVE-OPENCODE]: Orchestrator — real opencode provider', () => {
